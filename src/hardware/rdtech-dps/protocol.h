...
struct rdtech_dps_model {
	enum rdtech_dps_model_type _model_type;
	int id;
	const char *name;
	const struct rdtech_dps_range *x_ranges;
	size_t x_n_ranges;
};

struct etommens_etm_model {
...
struct model {
	enum rdtech_dps_model_type type;

	union {
		const struct rdtech_dps_model *rdtech;
		const struct etommens_etm_model *etm;
	};
};
...
struct dev_context {
	...
	struct rdtech_dps_range	dyn_range;
};
