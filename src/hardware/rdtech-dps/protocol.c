...
SR_PRIV void rdtech_dps_update_multipliers(const struct sr_dev_inst *sdi)
{
	...
	devc->power_multiplier = pow(10.0, range->power_digits);
}
...
SR_PRIV int rdtech_dps_update_range(const struct sr_dev_inst *sdi)
{
	 ...
	 */
	if (devc->n_ranges <= 1 || devc->acquisition_started)
		return SR_OK;
	if (devc->model.type != MODEL_RD)
		return SR_ERR;
	...
	struct rdtech_dps_state *state, enum rdtech_dps_state_context reason)
{
	...
	have_range = devc->n_ranges > 1;
	if (!have_range)
		range = 0;

	switch (devc->model.type) {
	case MODEL_DPS:
		/*
