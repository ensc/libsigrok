...
struct peaktech_device {
	...
	/* true iff serial connection is a poll source in the sdi session;
	 * switching states removes the source temporarily and adds it later
	 * in an "idle" callback */
	bool					has_source;
			       ...
			       struct sr_channel_group const *cg)
{
	...
	if (!cg) {
		...
		struct sr_channel const			*ch = cg->channels->data;
		size_t					idx = ch->index;
		struct peaktech_chan_config const	*cfg;

		g_assert(idx < devc->model->num_chan);
			       ...
			       struct sr_channel_group const *cg)
{
	...
	if (!cg) {
		...
		struct sr_channel const			*ch = cg->channels->data;
		size_t					idx = ch->index;
		struct peaktech_chan_config		*cfg;
		double					dval;

		if (idx >= ARRAY_SIZE(devc->config))
			return SR_ERR_ARG;
...
static gboolean peaktech_send_data_cb(int _fd, int revents, void *devc_)
{
	...
	if (revents == G_IO_OUT) {
		...
		len = serial_write_nonblocking(sdi->conn, ptr, cnt);
		if (len < 0) {
...
/**
 *  HACK: establishes poll sources.
 *
 *  We can not call serial_source_add() directly in the callback of an
 *  'sdi->conn' event but have to delay it which is done by running it in
 *  g_idle_add().
 */
static gboolean _peaktech_next_state_0(void *devc_)
{
	struct peaktech_device		*devc = devc_;
	struct sr_dev_inst		*sdi = devc ? devc->sdi : NULL;
	unsigned int			to;
	...
	case PEAKTECH_STATE_SEND:
		to = serial_timeout(sdi->conn, devc->send_len) + 50;
		serial_source_add(sdi->session, sdi->conn, G_IO_OUT, to,
				  peaktech_send_data_cb, devc);
		break;

	case PEAKTECH_STATE_EXPECT_CONFIRM:
		to = serial_timeout(sdi->conn, 2 * devc->send_len) + 100;
		serial_source_add(sdi->session, sdi->conn, G_IO_IN | G_IO_ERR,
				  to, peaktech_recv_confirm_cb, devc);
		break;

	case PEAKTECH_STATE_EXPECT_INQUIRY:
		to = serial_timeout(sdi->conn, devc->send_len + devc->model->reply_size) + 100;
		serial_source_add(sdi->session, sdi->conn, G_IO_IN | G_IO_ERR,
				  to, peaktech_recv_inquiry_cb, devc);
		break;
...
static void peaktech_next_state(struct peaktech_device *devc)
{
	...
	if (devc->state != PEAKTECH_STATE_INIT) {
		/* HACK: source is removed by returning 'FALSE' in the
		 * callbacks; new one is added in an idle callback below. */
		//serial_source_remove(sdi->session, sdi->conn);
		devc->has_source = false;
	}
	...
	case PEAKTECH_STATE_EXPECT_INQUIRY:
		/* HACK: we can not use
		 *
		 * g_timeout_add(serial_timeout(sdi->conn, devc->send_len),
		 *               _peaktech_next_state_0, devc);
		 *
		 * here because this terminates the main loop (probably
				  ...
				  struct peaktech_report *report)
{
	report->analog = (struct sr_datafeed_analog) {
		.data		= report->data,
		.num_samples	= devc->model->num_chan,
